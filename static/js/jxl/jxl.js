(function () {
      ...
      workers[jxlSrc].addEventListener("message", (m) => {
        if (m.data.url && m.data.blob) {
          ...
          config.useCache && cache &&
            cache.put(jxlSrc, new Response(m.data.blob));
        }
      });
    } else {
  ...
  async function decode(img, isCSS, isSource) {
    const jxlSrc = img.dataset.jxlSrc = isCSS
      ? getComputedStyle(img).backgroundImage.slice(5, -2)
      : isSource
      ? img.srcset
      : img.currentSrc;
    if (!isCSS && !isSource) {
      img.srcset = "";
      img.src =
        "data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="; // blank 1x1 image
    }
    if (config.useCache) {
      try {
      ...
      if (cachedImg) {
        ...
        requestAnimationFrame(() =>
          imgDataToDataURL(img, cachedImgData, isCSS, isSource)
        );
        return;
      }
    }
    ...
    workers[jxlSrc].addEventListener(
      "message",
      (m) =>
        m.data.imgData &&
        requestAnimationFrame(() =>
          imgDataToDataURL(img, m.data.imgData, isCSS, isSource)
        ),
    );
  }
    ...
    mutations.forEach((mutation) =>
      mutation.addedNodes.forEach((el) => {
        if (el instanceof HTMLImageElement && el.src.endsWith(".jxl")) {
          ...
          el instanceof HTMLSourceElement && el.srcset.endsWith(".jxl")
        ) {
